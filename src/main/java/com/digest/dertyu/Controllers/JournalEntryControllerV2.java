...
public class JournalEntryControllerV2 {
    ...
    public ResponseEntity<Optional<JournalEntity>>getbyId(@PathVariable ObjectId myId){
       // return journalEntryService.getByJournalId(myId).orElse(null);
        ...
        String name = authentication.getName();
        User user=userEntryService.findByUserName(name);
        List<JournalEntity> collect = user.getJournalEntities().stream().filter(x -> x.getId().equals(myId)).toList();
        if(!collect.isEmpty()){
    ...
    public ResponseEntity<Boolean>deleteById(@PathVariable ObjectId id){
        ...
        String name = authentication.getName();
        User user=userEntryService.findByUserName(name);
        List<JournalEntity> collect = user.getJournalEntities().stream().filter(x -> x.getId().
                equals(id)).toList();
        if(!collect.isEmpty()){
            try {
    ...
    public ResponseEntity<JournalEntity>updateBYId(@PathVariable ObjectId id,
        ...
        String name = authentication.getName();
        User byUserName = userEntryService.findByUserName(name);
        List<JournalEntity> collect = byUserName.getJournalEntities().stream().filter(x ->
                        x.getId().equals(id)).toList();
        if(!collect.isEmpty()){
            try {
